"""
Complete remaining EDA: returns distribution histogram, monthly returns
heatmap, ACF/PACF autocorrelation plots, and ADF stationarity test.

Outputs: plots/returns_distribution.png, plots/monthly_heatmap.png,
         plots/acf_pacf.png, plus key stats printed to console.
"""
...
def main():
    """Run remaining EDA: distributions, heatmap, ACF/PACF, ADF test."""
    ...
    ax.axvline(data["Returns"].mean(), color="red", linestyle="--",
    ...
    sns.heatmap(pivot, cmap="RdYlGn", center=0, annot=True, fmt=".2%", ax=ax,
    ...
    print("\n--- STATIONARITY TESTS ---")
    print(f"  Prices   ADF p-value: {result_price[1]:.6f}  "
          f"{'(Non-stationary)' if result_price[1] > 0.05 else '(Stationary)'}")
    print(f"  Returns  ADF p-value: {result_returns[1]:.6f}  "
